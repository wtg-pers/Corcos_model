%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [Weight] = Aerodynamic_MultiAirfoil_WeightWatcher(R,C,NBlades,method)
%
...
%
wd = ['./AirfoilOptimization/BoundaryLayerDataBase/'];
%
for airfoil=1:method.NFoilOpt

fid = fopen([wd 'Airfoil' sprintf('%03d',airfoil) '.dat'],'r');
...
iii=iii+1;
X2(iii,airfoil) = Xp(ii);
Y2(iii,airfoil) = Yp(ii);
end
NF=length(Xp);
...
%
for j=1:NP
   if (method.NFoilOpt==NP)
   airfoilindex=j;
   else
   airfoilindex=ceil(j/(NP/method.NFoilOpt));
   end
   %
   % EXTRADOS
   for i=1:N1
      X1_bis(i,j) = C(j)*X1(i,airfoilindex);
      Y1_bis(i,j) = C(j)*Y1(i,airfoilindex);
   end
   % INTRADOS
   for i=1:N2
      X2_bis(i,j) = C(j)*X2(i,airfoilindex);
      Y2_bis(i,j) = C(j)*Y2(i,airfoilindex);
   end
end
