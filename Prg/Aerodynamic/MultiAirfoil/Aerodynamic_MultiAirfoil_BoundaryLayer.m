...
function [boundary] = Aerodynamic_MultiAirfoil_BoundaryLayer(data,geom,method,k)

% load data '.mat' file
wd = ['./AirfoilOptimization/BoundaryLayerDataBase/'];
%
load([wd 'Airfoil' sprintf('%03d',k) '.mat']);
%
% read data
for i = 1 : size(BLayer,1) % for each Reynolds number
